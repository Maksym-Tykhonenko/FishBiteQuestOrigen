...
const FourthEasyLvl = ({navigation}) => {
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`FourthEasyLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setFoureEasyLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('FiveEasyLvl');
  };
