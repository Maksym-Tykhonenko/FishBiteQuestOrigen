...
const FirstEasyLvl = ({navigation}) => {
  ...
  console.log('FirstEasyLevelComplite==>', FirstEasyLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`FirstEasyLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setFirstEasyLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('SecondEasyLvl');
  };
