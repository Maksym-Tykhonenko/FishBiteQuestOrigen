...
const SecondEasyLvl = ({navigation}) => {
  ...
  console.log('SecondEasyLevelComplite==>', SecondEasyLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`SecondEasyLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setSecondEasyLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('ThirdEasyLvl');
  };
