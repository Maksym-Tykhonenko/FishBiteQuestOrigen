...
const SevenEasyLvl = ({navigation}) => {
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`SevenEasyLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setSevenEasyLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('EightEasyLvl');
  };
