...
const ThirdEasyLvl = ({navigation}) => {
  ...
  console.log('ThirdEasyLevelComplite==>', ThirdEasyLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`ThirdEasyLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setThirdEasyLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);
