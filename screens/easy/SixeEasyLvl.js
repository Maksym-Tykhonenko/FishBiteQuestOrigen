...
const SixeEasyLvl = ({navigation}) => {
  ...
  console.log('SixeEasyLevelComplite==>', SixeEasyLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`SixeEasyLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setSixeEasyLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);

  useEffect(() => {
    const shuffledFish = shuffleArray([...ocean]);
    const shuffledBait = shuffleArray([...ocean]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('SevenEasyLvl');
  };
