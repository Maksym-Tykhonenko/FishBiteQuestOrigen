...
import {australian} from '../../data/australian';
...
const EightHardLvl = ({navigation}) => {
  ...
  const [levelComplite, setLevelComplite] = useState(false);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setLevelComplite(true);
    }
  }, [correctPairs]);

  useEffect(() => {
    const shuffledFish = shuffleArray([...australian]);
    const shuffledBait = shuffleArray([...australian]);
