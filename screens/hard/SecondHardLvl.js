...
import {mediterraneanSea} from '../../data/mediterraneanSea';
...
const SecondHardLvl = ({navigation}) => {
  ...
  console.log('SecondHardLevelComplite==>', SecondHardLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`SecondHardLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setSecondHardLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);

  useEffect(() => {
    const shuffledFish = shuffleArray([...mediterraneanSea]);
    const shuffledBait = shuffleArray([...mediterraneanSea]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('ThirdHardLvl');
  };
