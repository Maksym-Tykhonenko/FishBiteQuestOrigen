...
const SevenHardLvl = ({navigation}) => {
  ...
  console.log('SevenHardLevelComplite==>', SevenHardLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`SevenHardLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setSevenHardLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);

  useEffect(() => {
    const shuffledFish = shuffleArray([...canada]);
    const shuffledBait = shuffleArray([...canada]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('EightHardLvl');
  };
