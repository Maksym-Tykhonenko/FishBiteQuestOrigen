...
const FirstHardLvl = ({navigation}) => {
  ...
  const [timeLeft, setTimeLeft] = useState(100); // Додаємо стан для таймера
  //
  const [FirstHardLevelComplite, setFirstHardLevelComplite] = useState(false);
  console.log('FirstHardLevelComplite==>', FirstHardLevelComplite);
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`FirstHardLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setFirstHardLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);

  useEffect(() => {
    const shuffledFish = shuffleArray([...caribian]);
    const shuffledBait = shuffleArray([...caribian]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('SecondHardLvl');
  };
