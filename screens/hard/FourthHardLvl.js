...
const FourthHardLvl = ({navigation}) => {
  ...
  const [timeLeft, setTimeLeft] = useState(85); // Додаємо стан для таймера
  //
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`FourthHardLvl`, jsonData);
  ...
  useEffect(() => {
    if (correctPairs.length >= 9) {
      ...
      setFoureHardLevelComplite(true);
      setShowInfo(true);
    }
  }, [correctPairs]);

  useEffect(() => {
    const shuffledFish = shuffleArray([...lake]);
    const shuffledBait = shuffleArray([...lake]);
  ...
  const goToNextLvl = () => {
    navigation.navigate('FiveHardLvl');
  };
